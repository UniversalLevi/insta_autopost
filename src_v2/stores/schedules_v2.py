"""
Isolated schedules store (schedules_v2). Keys: user_id, instagram_id.
"""
...
SCHEDULES_FILE = DATA_DIR / "schedules_v2.json"
...
def list_schedules(user_id: str, instagram_id: str) -> List[Dict[str, Any]]:
    data = _load_all()
    return data.get(_key(user_id, instagram_id), [])


def append_schedule(user_id: str, instagram_id: str, entry: Dict[str, Any]) -> None:
    data = _load_all()
